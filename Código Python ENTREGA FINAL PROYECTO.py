...
===========================================
- Profesora: Jacqueline Solís
- Estudiante: Gabriel Gutiérrez Brenes
===========================================
              DOCUMENTACIÓN
1. Pestañas de gestión:
- Usuario: línea 223
- Evento (RF-09): línea 455
- Categoría: línea 338
- Ubicación (RF-08): línea 663
- Disponibilidad (RF-11): línea 776
- Tarea (RF-15, RF-16, RF-17): línea 907

2. Consulta de traslape (RF-12): 585
===========================================
'''
...
class AppAgenda(ctk.CTk):
    ...
    def crear_area_principal(self):
        ...
        self.tab_tarea = self.tabview.add("Tareas")
        self.tab_estados = self.tabview.add("Estados")
        ...
        self.configurar_pestana_dispon()
        self.configurar_pestana_tarea()
        self.configurar_pestana_estados_tarea()
    ...
    def configurar_pestana_eventos(self):
        ...
        # Elegir ubicación del evento
        ctk.CTkLabel(form, text="Ubicación").pack(anchor="w", padx=10, pady=(8, 2))
        self.combo_ev_ub = ctk.CTkComboBox(form, values=["Seleccione una ubicación"], state="readonly")
        self.combo_ev_ub.set("Seleccione una ubicación")
        self.combo_ev_ub.pack(fill="x", padx=10, pady=4)
        self.cargar_ub_combo()
    ...
    def datos_evento_formulario(self):
        ...
        ubicacion = self.ub_combo.get(self.combo_ev_ub.get())
        try:
        ...
        if not titulo or usuario is None or categoria is None or ubicacion is None:
            raise ValueError("Completa título, propietario, categoría o ubicación.")
        if fin <= inicio:
    ...
    def agregar_evento(self):
        fecha_fin = self.obtener_fecha(self.fecha_fin)
        fecha_inicio = self.obtener_fecha(self.fecha_inicio)
        try:
            datos = self.datos_evento_formulario()
            traslape = self.ejecutar_consulta("""
                    SELECT COUNT(*)
                    FROM eventos
                    WHERE fecha_fin > %s AND %s > fecha_inicio;
                """, (fecha_fin, fecha_inicio), fetch=True)
            if(traslape[0][0]>0):
                messagebox.showinfo("Aviso","Hay traslape de horario con otro evento.")
    ...
    def cargar_ub_combo(self):
        try:
            rows = self.ejecutar_consulta("""
                ...
                ORDER BY nombre
            """, fetch=True) 
            valores_u = ["Seleccione una ubicación"] + list(self.ub_combo.keys())
            self.combo_ev_ub.configure(values=valores_u)
        except Exception as e:
    ...
    def configurar_pestana_ubicaciones(self):
        self.crear_encabezado(self.tab_ubicaciones, "Ubicaciones", "Gestión de ubicaciones donde se realizarán los eventos.")
        ...
        self.tree_ubicaciones.bind("<<TreeviewSelect>>", self.cargar_ubicacion_seleccionado)

        ctk.CTkLabel(form, text="Formulario de ubicación", font=ctk.CTkFont(size=16, weight="bold")).pack(pady=(10, 15))
        self.entry_nombre_ub = ctk.CTkEntry(form, placeholder_text="Nombre")
        self.entry_nombre_ub.pack(fill="x", padx=10, pady=6)
        self.entry_direccion_ub = ctk.CTkEntry(form, placeholder_text="Direccion")
        self.entry_direccion_ub.pack(fill="x", padx=10, pady=6)
        self.entry_ciudad_ub = ctk.CTkEntry(form, placeholder_text="Ciudad")
        self.entry_ciudad_ub.pack(fill="x", padx=10, pady=6)
        self.entry_capacidad_ub = ctk.CTkEntry(form, placeholder_text="Capacidad")
        self.entry_capacidad_ub.pack(fill="x", padx=10, pady=6)        
    ...
    def ubicacion_seleccionado_id(self):
        sel = self.tree_ubicaciones.selection()
        return self.tree_ubicaciones.item(sel[0])["values"][0] if sel else None

    def cargar_ubicacion_seleccionado(self, _=None):
        sel = self.tree_ubicaciones.selection()
        if not sel:
            return
        vals = self.tree_ubicaciones.item(sel[0])["values"]
        self.entry_nombre_ub.delete(0, tk.END); self.entry_nombre_ub.insert(0, vals[1])
        self.entry_direccion_ub.delete(0, tk.END); self.entry_direccion_ub.insert(0, vals[2])
        self.entry_ciudad_ub.delete(0, tk.END); self.entry_ciudad_ub.insert(0, vals[3])
        self.entry_capacidad_ub.delete(0, tk.END); self.entry_capacidad_ub.insert(0, vals[4])

    def limpiar_form_ubicacion(self):
        self.tree_ubicaciones.selection_remove(self.tree_ubicaciones.selection())
        self.entry_nombre_ub.delete(0, tk.END)
        self.entry_direccion_ub.delete(0, tk.END)
        self.entry_ciudad_ub.delete(0, tk.END)
        self.entry_capacidad_ub.delete(0, tk.END)

    def agregar_ubicacion(self):
        nombre, direccion, ciudad, capacidad = self.entry_nombre_ub.get().strip(), self.entry_direccion_ub.get().strip(), self.entry_ciudad_ub.get().strip(), self.entry_capacidad_ub.get().strip()
        if not nombre or not direccion or not ciudad or not capacidad:
            return messagebox.showwarning("Campos incompletos", "Indique el/los dato(s) faltante(s)")
        try:
    ...
    def actualizar_ubicacion(self):
        uid = self.ubicacion_seleccionado_id()
        if uid is None:
    ...
    def cargar_datos_ubicaciones(self):
        try:
            rows = self.ejecutar_consulta(
                "SELECT id_ubicacion, nombre, direccion, ciudad, capacidad FROM ubicaciones ORDER BY nombre, ciudad",
                fetch=True
            )
    ...
    def configurar_pestana_dispon(self):
        self.crear_encabezado(self.tab_disponibilidad, "Disponibilidad", "Registra la disponibilidad de un usuario con tipo, fecha y hora.")
        ...
        self.tree_dispon = self.crear_treeview(
            tabla_frame, ("Tipo", "Fecha inicio", "Fecha fin", "Hora inicio", "Hora fin"),
            (50, 20, 20, 20, 20)
        )
        self.tree_dispon.bind("<<TreeviewSelect>>", self.cargar_dispon_seleccionado)

        ctk.CTkLabel(form, text="Formulario de disponibilidad", font=ctk.CTkFont(size=16, weight="bold")).pack(pady=(10, 15))
        self.entry_tipo_dispon = ctk.CTkEntry(form, placeholder_text="Disponible o No disponible")
        self.entry_tipo_dispon.pack(fill="x", padx=10, pady=6)

        ctk.CTkLabel(form, text="Inicio").pack(anchor="w", padx=10, pady=(10, 2))
        fila_inicio_dispon = ctk.CTkFrame(form, fg_color="transparent"); fila_inicio_dispon.pack(fill="x", padx=10)
        self.fecha_inicio_dispon = self.crear_selector_fecha(fila_inicio_dispon)
        self.fecha_inicio_dispon.pack(side="left", fill="x", expand=True)
        self.hora_inicio_dispon = ctk.CTkEntry(fila_inicio_dispon, placeholder_text="HH:MM", width=75)
        self.hora_inicio_dispon.pack(side="left", padx=(6, 0))
        
        ctk.CTkLabel(form, text="Fin").pack(anchor="w", padx=10, pady=(10, 2))
        fila_fin_dispon = ctk.CTkFrame(form, fg_color="transparent"); fila_fin_dispon.pack(fill="x", padx=10)
        self.fecha_fin_dispon = self.crear_selector_fecha(fila_fin_dispon)
        self.fecha_fin_dispon.pack(side="left", fill="x", expand=True)
        self.hora_fin_dispon = ctk.CTkEntry(fila_fin_dispon, placeholder_text="HH:MM", width=75)
        self.hora_fin_dispon.pack(side="left", padx=(6, 0))

        ctk.CTkButton(form, text="➕ Registrar disponibilidad", command=self.agregar_dispon).pack(fill="x", padx=10, pady=(12, 5))
        ctk.CTkButton(form, text="💾 Actualizar seleccionado", command=self.actualizar_dispon).pack(fill="x", padx=10, pady=5)
        ctk.CTkButton(form, text="🧹 Nuevo / Limpiar", command=self.limpiar_form_dispon, fg_color="gray").pack(fill="x", padx=10, pady=5)
        ctk.CTkButton(form, text="🗑️ Eliminar seleccionado", command=self.eliminar_dispon, fg_color="#b33939", hover_color="#8f2d2d").pack(fill="x", padx=10, pady=5)

    def dispon_seleccionado_id(self):
        sel = self.tree_dispon.selection()
        return self.tree_dispon.item(sel[0])["values"][0] if sel else None

    def cargar_dispon_seleccionado(self, _=None):
        sel = self.tree_dispon.selection()
        if not sel:
            return
        vals = self.tree_dispon.item(sel[0])["values"]
        self.entry_tipo_dispon.delete(0, tk.END); self.entry_tipo_dispon.insert(0, vals[0])
        self.fecha_inicio_dispon.delete(0, tk.END); self.fecha_inicio_dispon.insert(0, vals[1])
        self.fecha_fin_dispon.delete(0, tk.END); self.fecha_fin_dispon.insert(0, vals[2])
        self.hora_inicio_dispon.delete(0, tk.END); self.hora_inicio_dispon.insert(0, vals[3])
        self.hora_fin_dispon.delete(0, tk.END); self.hora_fin_dispon.insert(0, vals[4])

    def limpiar_form_dispon(self):
        self.tree_dispon.selection_remove(self.tree_dispon.selection())
        self.entry_tipo_dispon.delete(0, tk.END)
        self.fecha_inicio_dispon.delete(0, tk.END)
        self.fecha_fin_dispon.delete(0, tk.END)
        self.hora_inicio_dispon.delete(0, tk.END)
        self.hora_fin_dispon.delete(0, tk.END)

    def agregar_dispon(self):
        tipo_d = self.entry_tipo_dispon.get().strip()
        fecha_i_dispon = self.obtener_fecha(self.fecha_inicio_dispon) 
        fecha_f_dispon = self.obtener_fecha(self.fecha_fin_dispon) 
        hora_i_dispon = self.hora_inicio_dispon.get().strip()
        hora_f_dispon = self.hora_fin_dispon.get().strip()
        if not tipo_d or not fecha_i_dispon or not fecha_f_dispon or not hora_i_dispon or not hora_f_dispon:
            return messagebox.showwarning("Campos incompletos", "Indique el/los dato(s) faltante(s).")
        try:
            self.ejecutar_consulta("INSERT INTO disponibilidad (tipo_disponibilidad, fecha_inicio, fecha_fin, hora_inicio, hora_fin) VALUES (%s, %s, %s, %s, %s)",
                                   (tipo_d,fecha_i_dispon,fecha_f_dispon,hora_i_dispon,hora_f_dispon))
            self.limpiar_form_dispon(); self.actualizar_todas_las_tablas()
            messagebox.showinfo("Éxito", "Disponibilidad registrada.")
        except Exception as e:
            messagebox.showerror("Error de base de datos", str(e))

    def actualizar_dispon(self):
        uid = self.dispon_seleccionado_id()
        if uid is None:
            return messagebox.showwarning("Selección requerida", "Seleccione una disponibilidad para actualizar.")
        tipo_d = self.entry_tipo_dispon.get().strip()
        fecha_i_dispon = self.fecha_inicio_dispon.get().strip()
        fecha_f_dispon = self.fecha_fin_dispon.get().strip() 
        hora_i_dispon = self.hora_inicio_dispon.get().strip()
        hora_f_dispon = self.hora_fin_dispon.get().strip()
        if not tipo_d or not fecha_i_dispon or not fecha_f_dispon or not hora_i_dispon or not hora_f_dispon:
            return messagebox.showwarning("Campos incompletos", "Indique el/los dato(s) faltante(s).")
        try:
            self.ejecutar_consulta("UPDATE disponibilidad SET tipo_disponibilidad=%s, fecha_inicio=%s, fecha_fin=%s, hora_inicio=%s, hora_fin=%s WHERE tipo_disponibilidad=%s",
                                   (tipo_d,fecha_i_dispon,fecha_f_dispon,hora_i_dispon,hora_f_dispon, uid))
            self.actualizar_todas_las_tablas()
            messagebox.showinfo("Éxito", "Disponibilidad actualizada.")
        except Exception as e:
            messagebox.showerror("Error", str(e))

    def eliminar_dispon(self):
        uid = self.dispon_seleccionado_id()
        if uid is None:
            return messagebox.showwarning("Selección requerida", "Seleccione una disponibilidad.")
        if not messagebox.askyesno("Confirmar", "¿Eliminar la disponibilidad seleccionada?"):
            return
        try:
            self.ejecutar_consulta("DELETE FROM disponibilidad WHERE tipo_disponibilidad=%s", (uid,))
            self.limpiar_form_dispon(); self.actualizar_todas_las_tablas()
            messagebox.showinfo("Eliminado", "Disponibilidad eliminada.")
        except Exception as e:
            messagebox.showerror("No se pudo eliminar", str(e))

    def cargar_datos_dispon(self):
        try:
            rows = self.ejecutar_consulta(
                "SELECT tipo_disponibilidad, fecha_inicio, fecha_fin, hora_inicio, hora_fin FROM disponibilidad ORDER BY tipo_disponibilidad, fecha_inicio",
                fetch=True
            )
            for item in self.tree_dispon.get_children(): self.tree_dispon.delete(item)
            self.dispon_combo = {}
            for row in rows:
                self.tree_dispon.insert("", "end", values=(row[0], row[1], row[2], row[3], row [4]))
                etiqueta = f"{row[1]} {row[2]} — #{row[0]}"
                self.dispon_combo[etiqueta] = row[0]
        except Exception as e:
            print(f"Error cargando disponibilidades: {e}")

    # -------------------- GESTIÓN DE TAREAS --------------------
    
    def configurar_pestana_tarea(self):
        self.crear_encabezado(self.tab_tarea, "Tareas", "Registro de tareas por título y fecha límite")
    
        cuerpo = ctk.CTkFrame(self.tab_tarea, fg_color="transparent")
        cuerpo.pack(fill="both", expand=True, padx=10, pady=5)
        cuerpo.grid_columnconfigure(0, weight=3)
        cuerpo.grid_columnconfigure(1, weight=1)
        cuerpo.grid_rowconfigure(0, weight=1)
        ...
        self.tree_tarea = self.crear_treeview(
            tabla_frame, ("Título", "Estado", "Prioridad", "Fecha límite", "Descripción"),
            (50, 20, 20, 20, 100)
        )
        self.tree_tarea.bind("<<TreeviewSelect>>", self.cargar_tarea_seleccionado)
    
        ctk.CTkLabel(form, text="Formulario de tarea", font=ctk.CTkFont(size=16, weight="bold")).pack(pady=(10, 15))
        self.entry_titulo_t = ctk.CTkEntry(form, placeholder_text="Título")
        self.entry_titulo_t.pack(fill="x", padx=10, pady=6)

        self.entry_estado_t = ctk.CTkEntry(form, placeholder_text="Estado (Pendiente - Progreso - Vencida)")
        self.entry_estado_t.pack(fill="x", padx=10, pady=6)
            
        self.entry_priori_t = ctk.CTkEntry(form, placeholder_text="Prioridad (Importante - No importante)")
        self.entry_priori_t.pack(fill="x", padx=10, pady=6)

        self.entry_desc_t = ctk.CTkEntry(form, placeholder_text="Descripción")
        self.entry_desc_t.pack(fill="x", padx=10, pady=6)

        ctk.CTkLabel(form, text="Fecha límite").pack(anchor="w", padx=10, pady=(10, 2))
        fila_fin_t = ctk.CTkFrame(form, fg_color="transparent"); fila_fin_t.pack(fill="x", padx=10)
        self.fin_t = self.crear_selector_fecha(fila_fin_t)
        self.fin_t.pack(side="left", fill="x", expand=True)
    
        ctk.CTkButton(form, text="➕ Registrar tarea", command=self.agregar_tarea).pack(fill="x", padx=10, pady=(12, 5))
    ...
    def tarea_seleccionado_id(self):
        sel = self.tree_tarea.selection()
        return self.tree_tarea.item(sel[0])["values"][0] if sel else None
    
    def cargar_tarea_seleccionado(self, _=None):
        sel = self.tree_tarea.selection()
        if not sel:
            return
        vals = self.tree_tarea.item(sel[0])["values"]
        self.entry_titulo_t.delete(0, tk.END); self.entry_titulo_t.insert(0, vals[0])
        self.entry_estado_t.delete(0, tk.END); self.entry_estado_t.insert(0, vals[1])
        self.entry_priori_t.delete(0, tk.END); self.entry_priori_t.insert(0, vals[2])
        self.fin_t.delete(0, tk.END); self.fin_t.insert(0, vals[3])
        self.entry_desc_t.delete(0, tk.END); self.entry_desc_t.insert(0, vals[4])
    
    def limpiar_form_tarea(self):
        self.tree_tarea.selection_remove(self.tree_tarea.selection())
        self.entry_titulo_t.delete(0, tk.END)
        self.entry_estado_t.delete(0, tk.END)
        self.entry_priori_t.delete(0, tk.END)
        self.fin_t.delete(0, tk.END) 
        self.entry_desc_t.delete(0, tk.END) 
    
    def agregar_tarea(self):
        titulo_t = self.entry_titulo_t.get().strip()
        estado_t = self.entry_estado_t.get().strip()
        priori_t = self.entry_priori_t.get().strip() 
        fin_t = self.fin_t.get().strip()
        desc_t = self.entry_desc_t.get().strip()
        if not titulo_t or not estado_t or not priori_t or not fin_t or not desc_t:
            return messagebox.showwarning("Datos incompletos", "Indique el/los dato(s) faltante(s).")
        try:
            self.ejecutar_consulta("INSERT INTO tareas (titulo, estado, prioridad, fecha_limite, descripcion) VALUES (%s, %s, %s, %s, %s)",
                                    (titulo_t,estado_t,priori_t,fin_t,desc_t))
    ...
    def actualizar_tarea(self):
        uid = self.tarea_seleccionado_id()
        if uid is None:
            return messagebox.showwarning("Selección requerida", "Seleccione una tarea para actualizar.")
        titulo_t = self.entry_titulo_t.get().strip()
        estado_t = self.estado_t.get().strip()
        priori_t = self.priori_t.get().strip() 
        fin_t = self.fin_t.get().strip()
        desc_t = self.desc_t.get().strip()
        if not titulo_t or not estado_t or not priori_t or not fin_t or not desc_t:
            return messagebox.showwarning("Campos incompletos", "Indique el/los dato(s) faltante(s).")
        try:
            self.ejecutar_consulta("UPDATE tareas SET titulo=%s, estado=%s, prioridad=%s, fecha_limite=%s, descripcion=%s WHERE titulo=%s",
                                    (titulo_t,estado_t,priori_t,fin_t,desc_t, uid))
            self.actualizar_todas_las_tablas()
    ...
    def eliminar_tarea(self):
        uid = self.tarea_seleccionado_id()
        if uid is None:
            return messagebox.showwarning("Selección requerida", "Seleccione una tarea.")
        if not messagebox.askyesno("Confirmar", "¿Eliminar la tarea seleccionada?"):
            return
        try:
            self.ejecutar_consulta("DELETE FROM tareas WHERE titulo=%s", (uid,))
            self.limpiar_form_tarea(); self.actualizar_todas_las_tablas()
    ...
    def cargar_datos_tarea(self):
        try:
            rows = self.ejecutar_consulta(
                "SELECT titulo, estado, prioridad, fecha_limite, descripcion FROM tareas ORDER BY fecha_limite",
                fetch=True
            )
            for item in self.tree_tarea.get_children(): self.tree_tarea.delete(item)
            self.tarea_combo = {}
            for row in rows:
                self.tree_tarea.insert("", "end", values=(row[0], row[1], row[2], row[3], row [4]))
                etiqueta = f"{row[1]} {row[2]} — #{row[0]}"
                self.tarea_combo[etiqueta] = row[0]
        except Exception as e:
            print(f"Error cargando tareas: {e}")    

    # -------------------- PESTAÑA DE ESTADOS TAREAS --------------------
    
    def configurar_pestana_estados_tarea(self):
        self.crear_encabezado(self.tab_estados, "Estados de Tareas", "Registro de tareas por estado (Pendiente - Progreso - Vencida)")
    
        cuerpo = ctk.CTkFrame(self.tab_estados, fg_color="transparent")
        cuerpo.pack(fill="both", expand=True, padx=10, pady=5)
        cuerpo.grid_columnconfigure(0, weight=3)
        cuerpo.grid_columnconfigure(1, weight=1)
        cuerpo.grid_rowconfigure(0, weight=1)
        ...
        self.tree_tarea_est= self.crear_treeview(
            tabla_frame, ("Pendiente", "Progreso", "Vencida"),
            (100, 100, 100)
        )

        self.tree_tarea_est.bind("<<TreeviewSelect>>", self.cargar_tarea_seleccionado)

    def cargar_datos_estados_tarea(self):
        try:
            rows = self.ejecutar_consulta(
                "SELECT titulo, estado FROM tareas ORDER BY titulo, estado",
                fetch=True
            )
            for item in self.tree_tarea_est.get_children(): self.tree_tarea_est.delete(item)
            
            for row in rows:
                titulo = row[0]
                estado = row[1]
                if(estado=="Pendiente"): value=(titulo,"","")
                elif(estado=="Progreso"): value=("",titulo,"")
                elif(estado=="Vencida"): value=("","",titulo)
                self.tree_tarea_est.insert("", "end", values=value)
    ...
    def actualizar_todas_las_tablas(self):
        ...
        self.cargar_datos_dispon()
        self.cargar_datos_tarea()
        self.cargar_datos_estados_tarea()
        self.cargar_ub_combo()
