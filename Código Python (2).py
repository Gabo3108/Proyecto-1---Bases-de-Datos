...
class AppAgenda(ctk.CTk):
    ...
    def configurar_pestana_dispon(self):
        self.crear_encabezado(self.tab_dispon, "Disponibilidad", "Registra la disponibilidad de un usuario con tipo, fecha y hora.")

        cuerpo = ctk.CTkFrame(self.tab_dispon, fg_color="transparent")
        cuerpo.pack(fill="both", expand=True, padx=10, pady=5)
        cuerpo.grid_columnconfigure(0, weight=3)
        cuerpo.grid_columnconfigure(1, weight=1)
        cuerpo.grid_rowconfigure(0, weight=1)
        ...
        self.entry_tipo_dispon = ctk.CTkEntry(form, placeholder_text="Tipo_dispon")
    ...
    def cargar_dispon_seleccionado(self, _=None):
        ...
        self.entry_tipo_dispon.delete(0, tk.END); self.entry_tipo_dispon.insert(0, vals[1])
        self.fecha_inicio_dispon.delete(0, tk.END); self.fecha_inicio_dispon.insert(0, vals[2])
        self.fecha_fin_dispon.delete(0, tk.END); self.fecha_fin_dispon.insert(0, vals[3])
        self.hora_inicio_dispon.delete(0, tk.END); self.hora_inicio_dispon.insert(0, vals[4])
        self.hora_fin_dispon.delete(0, tk.END); self.hora_fin_dispon.insert(0, vals[5])
    ...
    def agregar_dispon(self):
        self.entry_tipo_dispon.get()
        self.entry_tipo_dispon = self.fecha_inicio_dispon.get().strip(), self.fecha_fin_dispon.get().strip(), self.hora_inicio_dispon.get().strip(), self.hora_fin_dispon.get().strip()
        if not self.entry_tipo_dispon or not self.fecha_inicio_dispon or not self.fecha_fin_dispon or not self.hora_inicio_dispon or not self.hora_fin_dispon:
            return messagebox.showwarning("Campos incompletos", "Indique el/los dato(s) faltante(s).")
        try:
            self.ejecutar_consulta("INSERT INTO disponibilidad (tipo_disponibilidad, fecha_inicio, fecha_fin, hora_inicio, hora_fin) VALUES (%s, %s, %s, %s, %s)",
                                   (self.entry_tipo_dispon.get(), self.fecha_inicio_dispon.get(), self.fecha_fin_dispon.get(), self.hora_inicio_dispon.get(), self.hora_fin_dispon.get()))
    ...
    def actualizar_dispon(self):
        ...
        if not self.entry_tipo_dispon or not self.fecha_inicio_dispon or not self.fecha_fin_dispon or not self.hora_inicio_dispon or not self.hora_fin_dispon:
            return messagebox.showwarning("Campos incompletos", "Indique el/los dato(s) faltante(s).")
        try:
            self.ejecutar_consulta("UPDATE disponibilidad SET tipo_dispon=%s, fecha_inicio_dispon=%s, fecha_fin_dispon=%s, hora_inicio_dispon=%s, hora_fin_dispon=%s WHERE tipo_disponibilidad=%s",
                                   (self.entry_tipo_dispon.get().strip(),self.fecha_inicio_dispon.get().strip(),self.fecha_fin_dispon.get().strip(),self.hora_inicio_dispon.get().strip(),self.hora_fin_dispon.get().strip(),uid))
            self.actualizar_todas_las_tablas()
